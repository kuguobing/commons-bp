...
import org.robinku.commons.core.db.paging.dialect.DBPageDialect;
...
/**
 * A、Ibatis的分页都是通过滚动ResultSet实现的，应该算是逻辑分页。
...
public class PageSqlExecutor extends SqlExecutor implements Pagable {
    private static final Logger logger = LoggerFactory.getLogger(PageSqlExecutor.class);
    ...
    public void executeQuery(StatementScope statementScope, Connection conn, String sql, Object[] parameters,
        ...
        if ((skipResults != NO_SKIPPED_RESULTS || maxResults != NO_MAXIMUM_RESULTS) && supportPage()) {

            sql = this.dbPageDialect.getPageQuerySql(sql, skipResults, maxResults);
